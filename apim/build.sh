...
declare ARCH="noarch"
declare DESC="Gravitee.io API Management 1.x"
...
build_management_api() {
	...
	mkdir -p ${TEMPLATE_DIR}/opt/graviteeio/apim
        cp -fr .staging/graviteeio-full-${VERSION}/graviteeio-management-api-${VERSION} ${TEMPLATE_DIR}/opt/graviteeio/apim
	...
	docker run --rm -v "${PWD}:${DOCKER_WDIR}" -w ${DOCKER_WDIR} ${DOCKER_FPM}:rpm -t rpm \
                ...
                --rpm-group ${USER} \
                --rpm-attr "0750,${USER},${USER}:/opt/graviteeio" \
                --rpm-attr "0755,root,root:/etc/init.d/graviteeio-apim-management-api" \
                --directories /opt/graviteeio \
...
build_management_ui() {
	...
	docker run --rm -v "${PWD}:${DOCKER_WDIR}" -w ${DOCKER_WDIR} ${DOCKER_FPM}:rpm -t rpm \
                ...
                --rpm-group ${USER} \
                --rpm-attr "0750,${USER},${USER}:/opt/graviteeio" \
                --rpm-attr "0755,${USER},${USER}:/opt/graviteeio/apim/management-ui" \
                --directories /opt/graviteeio \
