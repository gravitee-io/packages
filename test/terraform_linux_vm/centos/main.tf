...
output "ssh_command_to_connect" {
  value = "ssh -i ${module.dependencies.random_pet-tf_resource_prefix-id}_id_rsa.pem -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no matthieu@${azurerm_linux_virtual_machine.rpm_vm.public_ip_address}"
}

output "scp_command_to_push_rpm" {
  value = "scp -i ${module.dependencies.random_pet-tf_resource_prefix-id}_id_rsa.pem -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ../../../apim/4.x/graviteeio-apim-*-4x*.rpm matthieu@${azurerm_linux_virtual_machine.rpm_vm.public_ip_address}:."
}
